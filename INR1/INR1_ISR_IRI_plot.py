...
# ==========================================
# 动态添加模块路径 (从 INR1/ 目录加载 inr_modules)
# ==========================================
...
try:
    ...
    print(f"  inr_modules 路径: {inr_module_path}")
    raise
...
def run_inr_iri_profile(model, sw_manager, gradient_bank, config, device):
    ...
    Returns:
        ...
        altitudes: 高度 km 数组
        inr_ne_2d: R-STMRF log10(Ne) 2D array (alt, time)
        iri_ne_2d: IRI proxy log10(Ne) 2D array (alt, time)
        log_var_ne_2d: R-STMRF 预测不确定性 2D array (alt, time)
    """
    ...
    time_hours = np.array([(t - ref_time).total_seconds() / 3600.0 for t in timestamps],
                          dtype=np.float32)
    ...
    for i in range(0, batch_size, batch_limit):
        ...
        # Get SW sequence: 逐点查询（每点对应其自身时间的历史窗口）
        ...
        # Run model
        with torch.no_grad():
            # Get IRI background (冻结的 IRI Neural Proxy)
            ...
            # Get R-STMRF prediction
            # forward 返回: (output, log_var, raw_correction, extra_outputs)
            output, log_var, correction, extras = model(batch_coords, sw_seq, tec_grad_direction)
            inr_pred = output.squeeze()
            log_var_pred = log_var.squeeze()

            inr_results.append(inr_pred.cpu().numpy())
            iri_results.append(iri_bg.cpu().numpy())
            log_var_results.append(log_var_pred.cpu().numpy())

    # Combine results
    inr_ne = np.concatenate(inr_results)
    iri_ne = np.concatenate(iri_results)
    log_var_ne = np.concatenate(log_var_results)

    inr_ne_2d = inr_ne.reshape(n_alts, n_times)       # shape: (alt, time)
    iri_ne_2d = iri_ne.reshape(n_alts, n_times)
    log_var_ne_2d = log_var_ne.reshape(n_alts, n_times)

    print(f"  R-STMRF log10(Ne) range: {np.nanmin(inr_ne_2d):.2f} - {np.nanmax(inr_ne_2d):.2f}")
    ...
    return timestamps, altitudes, inr_ne_2d, iri_ne_2d, log_var_ne_2d
...
def interpolate_to_isr_grid(model_timestamps, model_altitudes, model_ne, isr_data):
    ...
    # Convert ISR timestamps to same reference (strip timezone for arithmetic)
    ref = model_timestamps[0].to_pydatetime().replace(tzinfo=None)
    isr_times_num = np.array([
        (t.replace(tzinfo=None) - ref).total_seconds() / 3600.0
...
# ==========================================
def plot_5panel_comparison(inr_timestamps, inr_altitudes, inr_ne, iri_ne,
                           isr_data, inr_on_isr, iri_on_isr, config):
    """
    ...
    # =============================================
    # R-STMRF/IRI mesh
    inr_times_num = mdates.date2num(inr_timestamps.tolist())
    dt = np.diff(inr_times_num)
    time_edges_model = np.concatenate([
        [inr_times_num[0] - dt[0]/2],
        inr_times_num[:-1] + dt/2,
        [inr_times_num[-1] + dt[-1]/2]
    ])

    dh = np.diff(inr_altitudes)
    alt_edges_model = np.concatenate([
        [inr_altitudes[0] - dh[0]/2],
        inr_altitudes[:-1] + dh/2,
        [inr_altitudes[-1] + dh[-1]/2]
    ])

    T_model, H_model = np.meshgrid(time_edges_model, alt_edges_model)

    # ISR mesh
    isr_times_num = mdates.date2num([t.replace(tzinfo=None) for t in isr_data['timestamps']])
    ...
    vmin_ne = min(np.nanmin(inr_ne), np.nanmin(iri_ne), np.nanmin(isr_data['log_ne']))
    vmax_ne = max(np.nanmax(inr_ne), np.nanmax(iri_ne), np.nanmax(isr_data['log_ne']))

    # Difference color scale (for panels 4-5)
    diff_inr = inr_on_isr - isr_data['log_ne']
    diff_iri = iri_on_isr - isr_data['log_ne']
    vmax_diff = max(np.nanmax(np.abs(diff_inr)), np.nanmax(np.abs(diff_iri)))
    ...
    # =============================================
    ax2 = axes[1]
    pcm2 = ax2.pcolormesh(T_model, H_model, inr_ne,
                          cmap='jet', vmin=vmin_ne, vmax=vmax_ne, shading='flat')
    ...
    # =============================================
    ax4 = axes[3]
    pcm4 = ax4.pcolormesh(T_isr, H_isr, diff_inr,
    ...
    # Super title
    # =============================================
    date_str = inr_timestamps[0].strftime('%Y-%m-%d')
    fig.suptitle(
    ...
    valid_mask_inr = ~np.isnan(diff_inr)
    valid_mask_iri = ~np.isnan(diff_iri)

    if np.any(valid_mask_inr):
        rmse_inr = np.sqrt(np.nanmean(diff_inr**2))
        bias_inr = np.nanmean(diff_inr)
        mae_inr = np.nanmean(np.abs(diff_inr))
        print(f"  R-STMRF vs ISR:")
        print(f"    RMSE: {rmse_inr:.4f}")
        print(f"    Bias: {bias_inr:.4f}")
        print(f"    MAE:  {mae_inr:.4f}")
    ...
    if np.any(valid_mask_inr) and np.any(valid_mask_iri):
        print(f"\n  Improvement (R-STMRF over IRI):")
        print(f"    RMSE reduction: {(1 - rmse_inr/rmse_iri)*100:.1f}%")
        print(f"    MAE reduction:  {(1 - mae_inr/mae_iri)*100:.1f}%")
...
# ==========================================
def plot_uncertainty(inr_timestamps, inr_altitudes, log_var_ne, config):
    """
    Create separate figure for R-STMRF prediction uncertainty (log_var).
    log_var 是异方差不确定性估计头的输出 (log σ²)。
    """
    fig, ax = plt.subplots(1, 1, figsize=(14, 6))

    # Create mesh grid
    model_times_num = mdates.date2num(inr_timestamps.tolist())
    ...
    sigma = np.sqrt(np.exp(np.clip(log_var_ne, -20, 20)))
    print(f"  σ (std dev) mean: {np.nanmean(sigma):.4f} log10(Ne)")
    print("=" * 60)
...
def main():
    print("=" * 60)
    print("  R-STMRF vs IRI vs ISR 5-Panel Comparison")
    ...
    inr_on_isr = interpolate_to_isr_grid(inr_timestamps, inr_altitudes, inr_ne, isr_data)
    iri_on_isr = interpolate_to_isr_grid(inr_timestamps, inr_altitudes, iri_ne, isr_data)
    print("  Interpolation complete.")

    # 5. Plot 5-panel comparison
    print("\n" + "=" * 60)
    print("Generating 5-panel comparison figure...")
    fig = plot_5panel_comparison(
        inr_timestamps, inr_altitudes, inr_ne, iri_ne,
        isr_data, inr_on_isr, iri_on_isr, CONFIG
    )
    ...
    plot_uncertainty(inr_timestamps, inr_altitudes, log_var_ne, CONFIG)
