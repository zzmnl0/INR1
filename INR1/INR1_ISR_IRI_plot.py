"""
INR vs ISR vs IRI Comparison Visualization
==========================================
...
# Import model components from INR1.py
...
CONFIG = {
    ...
    'model_weights': r"D:\code11\IRI01\IRI03\INR1-1\checkpoints_r_stmrf\best_r_stmrf_model.pth",
...
def initialize_model(config):
    """Initialize INR model and managers."""
    ...
    # Physics-Guided INR
    print("Initializing Physics-Guided INR Model...")
    ...
    print("Loading trained INR weights...")
...
# ==========================================
# INR and IRI Inference for Time-Altitude Profile
# ==========================================
def run_inr_iri_profile(model, sw_manager, tec_manager, config, device):
    """
    Run INR and IRI inference for a single location over time and altitude.
    Returns: timestamps (datetime), altitudes (km), INR log10(Ne), IRI log10(Ne)
    """
    ...
    print(f"INR/IRI Inference Grid:")
    ...
    print(f"  INR log10(Ne) range: {np.nanmin(inr_ne_2d):.2f} - {np.nanmax(inr_ne_2d):.2f}")
...
def interpolate_to_isr_grid(model_timestamps, model_altitudes, model_ne, isr_data):
    """
    Interpolate model data (INR or IRI) to ISR time-altitude grid.
    """
...
def plot_5panel_comparison(inr_timestamps, inr_altitudes, inr_ne, iri_ne, 
    ...
    # =============================================
    # INR/IRI mesh
    inr_times_num = mdates.date2num(inr_timestamps)
    ...
    ax2.set_title('(b) Physics-Guided INR Prediction', fontsize=12, fontweight='bold', loc='left')
    ...
    ax4.set_title('(d) Difference: INR − ISR', fontsize=12, fontweight='bold', loc='left')
    ...
    fig.suptitle(
        f'INR vs IRI vs ISR Comparison - Jicamarca ({config["target_lat"]}°N, {config["target_lon"]}°E)\n{date_str}', 
    ...
    # Save figure
    # =============================================
    output_path = os.path.join(config['output_dir'], 'inr_iri_isr_5panel_comparison.png')
...
def main():
    print("=" * 60)
    print("  INR vs IRI vs ISR 5-Panel Comparison")
    ...
    # 2. Run INR and IRI inference
    print("\n" + "=" * 60)
    print("Running INR and IRI inference...")
    inr_timestamps, inr_altitudes, inr_ne, iri_ne = run_inr_iri_profile(
        model, sw_manager, tec_manager, CONFIG, device
    )
    ...
    # 4. Interpolate INR and IRI to ISR grid
    print("\n" + "=" * 60)
